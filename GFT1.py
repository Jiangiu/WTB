...
# class FrGTConv1(nn.Module):
#     def __init__(
#             self, in_channels, out_channels, order=0.25, filter="FrFT", attention="ca"
#     ):
#         super().__init__()
#         self.PWC0 = Conv(in_channels, in_channels // 2, 1)
#         self.PWC1 = Conv(in_channels, out_channels, 1)
#         self.SPU = SPU(in_channels // 2, out_channels)
#
#         assert filter in (
#             "FrFT",
...
#         ), "The filter type must be either Fractional Fourier Transform(FrFT) or Fractional Gabor Transform(FrGT)."
#         if filter == "FrFT":
#             self.FPU = FourierFPU(out_channels, out_channels, order)
#         elif filter == "FrGT":
#             self.FPU = GaborFPU(out_channels, out_channels, order)
#         self.c3 = Conv(out_channels, out_channels, 1)
#
#         assert attention in (
#             "sa",
...
#         # feature1 = self.SPU(x1)
#         feature2 = self.FPU(x2)
#         if feature2.shape[1] == x.shape[1]:
#               feature_out = feature2 + x
#         return feature_out
#         # Spatial Attention
#         # if self.attention == "sa":
#         #     feature_U = feature1 + feature2
...
class FrGTConv1(nn.Module):
    def __init__(
            self, in_channels, out_channels, order=32, filter="FrFT", attention="ca"
    ):
        super().__init__()
        self.PWC0 = Conv(in_channels, in_channels // 2, 1)
    ...
    def forward(self, x):
        x2 = self.PWC1(x)
        feature_out = self.FPU(x2)
        if feature_out .shape[1] == x.shape[1]:
            feature_out = feature_out+ x
            # feature_out = self.bn(feature_out)
            # feature_out = self.rele(feature_out)
        return feature_out
        # # Spatial Attention
        # if self.attention == "sa":
        #     feature_U = feature1 + feature2
...
class FT_Conv_Concat(nn.Module):
    def __init__(self, in_channels, out_channels, order=0.25, filter="FrFT", attention="ca",p=None, g=1, k=1):
        super().__init__()

        # 使用您现有的 FT_Conv 模块
        self.ft_conv = FrGTConv1(out_channels, out_channels//2, order=8, filter="FrFT", attention="ca")
        self.ft_conv1 = FrGTConv1(out_channels, out_channels, order=256, filter="FrFT", attention="ca")
        self.ft_conv2 = FrGTConv1(out_channels, out_channels, order=128, filter="FrFT", attention="ca")
        self.ft_conv3 = FrGTConv1(out_channels, out_channels, order=128, filter="FrFT", attention="ca")
        self.ft_conv4 = FrGTConv1(out_channels, out_channels, order=64, filter="FrGT", attention="ca")
        self.ft_conv5 = FrGTConv1(out_channels//2, out_channels//2, order=16, filter="FrGT", attention="ca")
        self.ft_conv6 = FrGTConv1(out_channels, out_channels, order=16, filter="FrGT", attention="ca")
        self.ft_conv7 = FrGTConv1(out_channels, out_channels, order=32, filter="FrGT", attention="ca")
        # 串联的卷积层
        self.conv2 = nn.Conv2d(out_channels, out_channels, k,  groups=g, bias=False)  # 此处参数 (out_channels, out_channels, 3) 仅作示例
        self.conv3 = nn.Conv2d(in_channels, out_channels, k,  groups=g, bias=False)
        self.conv4 = nn.Conv2d(out_channels, out_channels, k,  groups=g, bias=False)
        self.conv1 = Conv(in_channels, out_channels, k=1, s=1)  # 此处参数 (out_channels, out_channels, 3) 仅作示例
        self.pool = nn.AdaptiveMaxPool2d(1)
        self.relu = nn.ReLU()
    def forward(self, x):
        x1 = self.conv3(x)# FT_Conv 模块的输出 # 串联的卷积层的输出
        x2 = self.ft_conv3(x1)
